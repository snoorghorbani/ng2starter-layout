...
var ToolbarMenuComponent = /** @class */ (function () {
    function ToolbarMenuComponent(document, _location, store, configurationService) {
        ...
        this.config$ = this.store.select(reducers_1.getLayoutToolbar);
        ...
        fromEvent_1.fromEvent(this.document.body, "scroll").subscribe(function () {
            debugger;
            var scrolledAmount = _this.document.body.scrollTop;
            if (scrolledAmount == 0) {
                if (_this.config.mode == "comfortable")
                    return;
                _this.store.dispatch(new actions_1.ChangeToolbatToComfortableModeAction());
            }
            else if (scrolledAmount < 128) {
                if (_this.config.mode == "compact")
                    return;
                _this.store.dispatch(new actions_1.ChangeToolbatToCompactModeAction());
            }
            else if (scrolledAmount > _this.lastScroll) {
                if (_this.config.mode == "summary")
                    return;
                _this.store.dispatch(new actions_1.ChangeToolbatToSummaryModeAction());
            }
            else {
                if (_this.config.mode == "compact")
                    return;
